...
if(isset($email)){
    if(isset($email)){
        ...
        $sql = mysqli_query($connect, "SELECT * FROM perfil WHERE email = '{$emailPostado}'") or die(mysqli_error($connect));

        // se ja tiver um email
        ...
        if (mysqli_num_rows($sql)>0){
            ...
            $perfil = mysqli_query($connect, $perfil);
            ...
            $comp  = mysqli_query($connect, $comp );
...
<script>
    alert("cadastro erealizado com sucesso");
    window.location="index.php";
</script>